...
trait PaymentTrait {
	...
	public function getPaymentDetails($borrower, $amount) 
    {
        $MERCHANT_KEY=env('MERCHANT_LIVE_KEY'); //env key
        $application = $borrower->application;
        $payment_details= [];
        $ENV =env('APP_URL');
        $payment_details['key'] = $MERCHANT_KEY;
        $payment_details['txnid'] = substr(hash('sha256', mt_rand() . microtime()), 0, 20);
        $payment_details['amount'] = $amount;
        $payment_details['firstname'] = $borrower->first_name;
        $payment_details['email'] = $borrower->email;
        $payment_details['phone'] = $borrower->mobile_number;
        $payment_details['udf1'] = $application->id;
        $payment_details['udf2'] = $application->card_no;
        $payment_details['udf3'] = $application->reference_code;
        $payment_details['udf4'] = $application->pin_number;
        $payment_details['udf5'] = $application->treatment_type;

        if ($application->type=="card") {
            $payment_details['productinfo'] ="Arogya Card";
        }
        
        if ($application->type=="loan") {
            $payment_details['productinfo'] ="Arogya Loan";
        }
            
        $payment_details['surl'] = $ENV."/payment/successful/{$application->id}"; 
        $payment_details['furl'] = $ENV."/payment/failed/{$application->id}";
